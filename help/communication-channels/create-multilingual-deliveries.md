---
title: 多言語メールまたはSMS配信の作成
description: 多言語キャンペーンを送信する際の操作の複雑さを簡略化する方法について説明します。
feature: Email, SMS
jira: KT-1277
thumbnail: 23252
doc-type: feature video
activity: use
team: TM
exl-id: 4b7e4602-6d4f-4a16-bbe7-070a95511ebb
role: User
level: Intermediate, Experienced
TQID: https://experienceleague.adobe.com/xYTtGCBZJEgvHLd50LA-OWupnlADy9AWwwtsBgaB7bw
product_v2: id: dfc56824-e8b9-499e-85d4-21aedb507314
feature_v2: id: a075b2c1-7748-4328-b7f6-343aa314616a
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2: id: b5a62a22-46f7-4f0d-b151-3fc640bef588
source-git-commit: 7d15c0a5dc01907ff529b3684eaddaca5321facc
workflow-type: tm+mt
source-wordcount: 101
ht-degree: 0%

---

# 多言語メールまたはSMS配信の作成

Adobe Campaignを使用すると、多言語施策を送信する際の操作の複雑さを軽減できます。 メール配信とSMS配信はどちらも多言語として設定でき、プロファイルの好みの言語に基づいてメールやSMSを送信できます。

このビデオでは、メールニュースレターの例に基づいて、テンプレートに基づいて多言語配信を作成する方法を説明します。 動画で紹介されているアプローチは、電子メール配信とSMS配信の両方に適用されます。

>[!VIDEO](https://video.tv.adobe.com/v/23252?learn=on){transcript=true}
